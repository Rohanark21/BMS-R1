...
% Data Extraction
A=max(results{1}.maxTorque)


function results = setupSimVehicle(grade)
  addpath readonly
  files = {'mumbai.txt','udds.txt','cs06.txt','hwy.txt'};
...
function results = simVehicle(vehicle,cycle,grade)
  ...
  if isscalar(grade),
    ...
    results.grade = atan(grade/100);
  end
  results.desSpeedKPH = cycle(:,2) * 1.609344;
  results.desSpeed = min(vehicle.maxSpeed,results.desSpeedKPH*1000/3600);
...
function module = setupModule(numParallel,numSeries,overhead,cell)
  ...
  module.capacity = numParallel * cell.capacity; %Ah
